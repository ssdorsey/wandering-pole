...
saveRDS(mRT, file='~/Dropbox/Projects/Twitter/rtMod_polarizing_newAPI.RData'); rm(mRT); gc()
mFave <- lm(faveDiff ~ polarizing + memberPresPty + ideolDiffPos + absPVI + house + female + years + congress + icpsr, data=tw)
saveRDS(mFave, file='~/Dropbox/Projects/Twitter/faveMod_polarizing_newAPI.RData'); rm(mFave); gc()
...
mRT <- readRDS('~/Dropbox/Projects/Twitter/rtMod_polarizing_newAPI.RData')
mFave <- readRDS('~/Dropbox/Projects/Twitter/faveMod_polarizing_newAPI.RData')
