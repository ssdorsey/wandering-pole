...
Followerincrease <- c(pol, tw, pct)
ses <- c(polse, twse, pctse)
...
followercoefs$Followerincrease_ci  <- as.numeric(followercoefs$ses * 1.96)
followercoefs$Followerincrease_low <- as.numeric((followercoefs$Followerincrease - (followercoefs$ses * 1.96)))
followercoefs$Followerincrease_up <- as.numeric((followercoefs$Followerincrease + (followercoefs$ses * 1.96)))
...
Followerincrease2 <- c(pol2, tw2, pct2)
ses2 <- c(polse2, twse2, pctse2)
...
followercoefs2$Followerincrease_ci  <- as.numeric(followercoefs2$ses2 * 1.96)
...
barplot(followercoefs$Followerincrease, ylim=c(0, 50), ylab='Weekly Numerical Change in Twitter Followers', names.arg=c('# Polarizing', '# Tweets', '% Polarizing'))
for(ii in 1:3){
  ...
  text(x=barCenters[ii], y=followercoefs$Followerincrease[ii]/2, paste0(round(followercoefs$Followerincrease[ii], 2), ' +/- ', round(followercoefs$Followerincrease_ci[ii], 2)))
}
...
par(mar=c(3.1, 4.1, 2.1, 1.1))
barplot(followercoefs2$Followerincrease, ylim=c(0, 0.025), ylab='Weekly Percent Change in Twitter Followers', names.arg=c('# Polarizing', '# Tweets', '% Polarizing'))
for(ii in 1:3){
  ...
  text(x=barCenters[ii], y=followercoefs2$Followerincrease[ii]/2, paste0(round(followercoefs2$Followerincrease[ii], 3), ' +/- ', round(followercoefs2$Followerincrease_ci[ii], 3)))
}
par(mar=c(5.1, 4.1, 4.1, 2.1))
dev.off()


#calculate standard deviations for comparison

mean(tweets$followerchange)
mean(tweets$followerchange_pct)

sd(tweets$followerchange)
sd(tweets$followerchange_pct)

sd(tweets$polarizing)
